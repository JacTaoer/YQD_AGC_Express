...
启动 DevEco Studio 模拟器后执行：

```bash
./tools/start-atomic-service.sh
```

脚本会安装并启动 `atomic` product 生成的元服务 HAP。右上角系统胶囊由鸿蒙元服务宿主注入，需要从模拟器的元服务/服务中心入口打开；直接启动 Ability 只能验证页面，系统不会显示胶囊。

本工程按官方 ShoppingTemplate/Express README 作为普通 HarmonyOS 应用运行。
在 AppGallery Connect 创建应用后，请确保控制台包名与 `AppScope/app.json5` 中的
`com.atomicservice.6917614461242904059` 完全一致，并将入口模块中的 `__HUAWEI_CLIENT_ID__` 替换为该应用的 OAuth 2.0
`client_id`。

发布或真机调试需要手工签名时，将证书文件放在本机安全目录，并把
`huawei-signing.example.json5` 中的 `material` 内容复制到根目录 `build-profile.json5`
的 `signingConfigs` 数组。需要的文件是 `.cer`、`.p7b` 和 `.p12`；私钥库密码、密钥别名和
密钥密码只保存在本机，不要提交到仓库。最后在 AppGallery Connect 的“证书指纹”中添加
该 `.cer` 对应的 SHA-256 指纹。
